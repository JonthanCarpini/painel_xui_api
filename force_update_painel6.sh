#!/bin/bash
# Update agressivo: git reset, docker build --no-cache, restart painel_6

REPO_DIR="/opt/painel_xui_api"
IMAGE_NAME="carpini/painelshark:latest"

echo "=== 1. Atualizando repositório (Forçado) ==="
cd "$REPO_DIR"
git fetch origin
...
echo "=== 3. Reiniciando painel_6 ==="
docker stop painel_6 2>/dev/null
docker rm painel_6 2>/dev/null
...
echo "=== 4. Limpando caches internos ==="
docker exec painel_6 php artisan optimize:clear
docker exec painel_6 php artisan view:clear
docker exec painel_6 rm -rf /var/www/storage/framework/views/*
