...
<div class="w-full">
    ...
    {{-- Listas por per&iacute;odo --}}
    @foreach($groups as $type => $group)
    @if(count($group['clients']) > 0)
    <div class="bg-white dark:bg-dark-300 border border-gray-200 dark:border-dark-200 rounded-xl overflow-hidden shadow-sm mb-6">
        <div class="border-b border-gray-200 dark:border-dark-200 px-6 py-4 bg-gray-50 dark:bg-dark-200/50 flex items-center justify-between">
            <h3 class="text-base font-bold text-gray-900 dark:text-white flex items-center gap-2">
                <i class="bi {{ $group['icon'] }} text-{{ $group['color'] }}-500"></i>
                {{ $group['label'] }}
                <span class="ml-2 px-2 py-0.5 bg-{{ $group['color'] }}-100 dark:bg-{{ $group['color'] }}-500/10 text-{{ $group['color'] }}-600 dark:text-{{ $group['color'] }}-400 rounded-full text-xs font-bold">{{ count($group['clients']) }}</span>
            </h3>
        </div>
                ...
                <tbody class="divide-y divide-gray-50 dark:divide-dark-200">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-dark-200/50 transition-colors">
                        ...
                        <td class="px-6 py-3">
                            ...
                            @else
                            <span class="text-gray-400 text-xs italic">N&atilde;o cadastrado</span>
                            @endif
                        </td>
                        <td class="px-6 py-3">
                        ...
                        <td class="px-6 py-3">
                            ...
                            <span class="text-gray-600 dark:text-gray-400 text-xs">
                                ...
                                <i class="bi bi-clock text-yellow-500"></i> Previs&atilde;o: {{ $client['sent_at'] instanceof \Carbon\Carbon ? $client['sent_at']->format('d/m/Y \à\s H:i:s') : $client['sent_at'] }}
                                @else
    ...
    {{-- Mensagem quando n&atilde;o h&aacute; clientes --}}
    @if(collect($groups)->sum(fn($g) => count($g['clients'])) === 0)
    <div class="bg-white dark:bg-dark-300 border border-gray-200 dark:border-dark-200 rounded-xl p-12 text-center shadow-sm">
        ...
        <h4 class="text-lg font-bold text-gray-900 dark:text-white mb-2">Nenhuma notifica&ccedil;&atilde;o pendente</h4>
        <p class="text-gray-500 dark:text-gray-400 text-sm">N&atilde;o h&aacute; clientes com vencimento nos pr&oacute;ximos 3 dias.</p>
    </div>
    @endif
</div>
@endsection
