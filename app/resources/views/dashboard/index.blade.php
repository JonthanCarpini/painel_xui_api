...
<script>
    ...
    const dashBouquetNames = @json(collect($bouquets)->pluck('bouquet_name', 'id'));
    const dashUnitMap = { 'hours':'hora(s)','hour':'hora(s)','days':'dia(s)','day':'dia(s)','months':'mês(es)','month':'mês(es)','years':'ano(s)','year':'ano(s)' };
    const dashOutputNames = { 1:'HLS', 2:'MPEGTS', 3:'RTMP' };
    ...
    function updateTrialPackage(select) {
        ...
        document.getElementById('trialDurationDisplay').value = `${duration} ${dashUnitMap[durationIn] || durationIn}`;
        ...
        try {
            ...
            document.getElementById('trialOutputDisplay').value = formats.map(id => dashOutputNames[id] || `#${id}`).join(', ');
        ...
        try {
            ...
            if (bIds.length > 0) {
                const tags = bIds.map(id => {
                    const name = dashBouquetNames[id] || `Bouquet #${id}`;
            ...
            } else { el.innerHTML = 'Nenhum buquê no pacote'; }
