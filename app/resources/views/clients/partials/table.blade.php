    ...
    <table class="w-full">
        ...
        <tbody class="divide-y divide-dark-200">
            ...
            <tr class="hover:bg-dark-200/50 transition-colors">
                ...
                <td class="px-4 py-4">
                    <span class="text-gray-300">{{ $client->password }}</span>
                </td>
                <td class="px-4 py-4">
                    <span class="text-gray-300">{{ $client->admin_notes ?? '-' }}</span>
                </td>
                <td class="px-4 py-4">
                    <span class="text-gray-300">{{ $client->member->username ?? 'N/A' }}</span>
                </td>
                <td class="px-4 py-4">
                    @if($client->is_trial)
                ...
                <td class="px-4 py-4">
                    <span class="text-gray-400 text-sm">{{ date('d/m/Y, H:i', $client->created_at) }}</span>
                </td>
                <td class="px-4 py-4">
                    @php
                ...
                <td class="px-4 py-4">
                    @if($client->enabled && $client->admin_enabled && $client->exp_date > time())
                ...
                <td class="px-4 py-4">
                    <div class="flex gap-2">
                        <button onclick="window.location.href='/clients/{{ $client->id }}/m3u'" class="p-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg transition-colors" title="M3U">
                            <i class="bi bi-file-earmark-code"></i>
                        </button>
                        <button onclick="openEditModal({{ $client->id }})" class="p-2 bg-gray-600 hover:bg-gray-700 text-white rounded-lg transition-colors" title="Editar">
                            <i class="bi bi-pencil"></i>
                        </button>
                        <button onclick="openRenewModal({{ $client->id }}, '{{ $client->username }}')" class="p-2 bg-green-600 hover:bg-green-700 text-white rounded-lg transition-colors" title="Renovar">
                            <i class="bi bi-arrow-clockwise"></i>
                        </button>
                        <button onclick="window.open('https://wa.me/{{ preg_replace('/[^0-9]/', '', $client->contact ?? '') }}', '_blank')" class="p-2 bg-orange-500 hover:bg-orange-600 text-white rounded-lg transition-colors" title="WhatsApp">
                            <i class="bi bi-whatsapp"></i>
                        </button>
                        <button onclick="window.location.href='/clients/{{ $client->id }}/logs'" class="p-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg transition-colors" title="Logs">
                            <i class="bi bi-clock-history"></i>
                        </button>
                        <button onclick="confirmDelete({{ $client->id }})" class="p-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition-colors" title="Excluir">
